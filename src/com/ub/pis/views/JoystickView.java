...
public class JoystickView extends View {
	...
	private double[] baseVector; // Vector base que parteix desde la dreta
									// v(baseRadius,0) tenint el origen de
	...
	public static interface OnJoystickChangeListener {
		...
		/**
		 * Se llamará a este evento cuando se haya dejado de apretar al joystick, siempre y que antes se haya movido
		 * 
		 */
	...
	protected void onDraw(Canvas canvas) {
		canvas.save();
		if (principi) {
			baseRadius = (int) TypedValue.applyDimension(
			...
			rectSupMig = new Rect(rectLimit.left, 0, rectLimit.right,
					rectLimit.top);
			rectSupDreta = new Rect(rectLimit.right, 0, getWidth(),
					rectLimit.top);
			rectCentreEsquerra = new Rect(0, rectLimit.left, rectLimit.left,
					rectLimit.bottom);
			rectCentreDreta = new Rect(rectLimit.right, rectLimit.top,
					getWidth(), rectLimit.bottom);
			rectInfEsquerra = new Rect(0, rectLimit.bottom, rectLimit.left,
					getHeight());
			rectInfMig = new Rect(rectLimit.left, rectLimit.bottom,
					rectLimit.right, getHeight());
			rectInfDreta = new Rect(rectLimit.right, rectLimit.bottom,
					getWidth(), getHeight());
	...
	private void procesarMovimientJoystick(float posX, float posY) {
		...
		if (abs > baseRadius-palancaRadius) {
			...
			if (listener != null) {
				listener.onValueChanged((int) angulo);
	...
	/**
	 * Cuando se aplica el listener, debe especificarse cada cuanto se llamará a las funciones implementadas
	 * 
	 * @param listener
	 * @param millis
	 */
